...
class Filestore(Protocol):
    def save(self,filename,content)->int:
        ...
...
class LegacyFilestoreAdapter:
    def __init__(self,legacy_file_store:LegacyFileStore):
        self.legacy_file_store=legacy_file_store
        self._map_file_id_to_path:Dict[int, str]={}
        self.file_id=0

    def save(self,filename,content)->int:
        path=f"/file/{self.file_id}/{filename}"
        self.legacy_file_store.put_file(path=path,data=content)
        self._map_file_id_to_path[self.file_id]=path
        self.file_id+=1
        return self.file_id-1
    def load(self,file_id):
        path=self._map_file_id_to_path[file_id]
        file=self.legacy_file_store.read_file(path=path)
        return file
    def delete(self,file_id):
        path=self._map_file_id_to_path[file_id]
        result=self.legacy_file_store.remove_file(path)
        if result=="REMOVED":
            del self._map_file_id_to_path[file_id]
            return True
        return False
...
class UserFileService:
    ...
    def save_it(self,filename,content):
        return self.filestore.save(filename,content)

    def load_it(self,file_id):
        return self.filestore.load(file_id)
    
    def delete(self,file_id):
        return self.filestore.delete(file_id)

legacy=LegacyFileStore()
adapter=LegacyFilestoreAdapter(legacy)
userfile=UserFileService(adapter)

userfile.save_it("nahom","this is the content inside nahom file")
